...
int sub2ind(int numRows, int rowSub, int colSub)
{
	int i = (numRows * rowSub);
	int j = i + colSub;
	return j;
}
...
namespace CIRAFI
{
	void CIRAFIData::CountParameter(Mat& templateImage)
	{
		...
		_angleRadian = _angleDegree * PI / 180.0;
	...
	void CIRAFIData::Rafi(Mat& sourceImage)
	{
		...
		for (int i = 0; i<_cis.size(); i++)
		{
			...
			double angleRange = 2.0*PI / _angleNum;
	...
	Mat CIRAFIData::DrawTefiResult(Mat& sourceImage, double sampleRatio)
	{
		...
		for (int i = 0; i<_tes.size(); i++)
		{
			...
			double angle = angleNO*_angleRadian + (PI / 2);
